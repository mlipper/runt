...
class PDateTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @day_prec = PDate.day_of_month(2004,1,25,11,39)
    #11:59(:04 - ignored), December 31st, 1999
    @minute_prec = PDate.minute(1999,12,31,23,59,4)
    #12:00:10 am, March 1st, 2004
    @second_prec = PDate.second(2004,3,1,0,0,10)
  end
  ...
  def test_new
    ...
    date2 = PDate.day_of_month(2004,2,29)
    assert(date==date2)

    date_time2 = PDate.second(2004,2,29,22,13,2)
  ...
  def test_plus
    ...
    assert(PDate.day_of_month(2004,2,1)==(@day_prec+7))
    assert(PDate.minute(2000,1,1,0,0)==(@minute_prec+1))
    assert(PDate.second(2004,3,1,0,0,21)==(@second_prec+11))
  end
  ...
  def test_minus
    ...
    assert(PDate.day_of_month(2003,12,30)==(@day_prec-26))
    assert(PDate.minute(1999,12,31,21,57)==(@minute_prec-122))
    assert(PDate.second(2004,2,29,23,59,59)==(@second_prec-11))
  end
  def test_spaceship_comparison_operator
    sec_prec = PDate.second(2002,8,28,6,04,02)
    assert(PDate.year(1998,12)<sec_prec)
    assert(PDate.month(2002,9)>sec_prec)
    assert(PDate.day_of_month(2002,8,28)==sec_prec)
    assert(PDate.minute(1999,12,31,21,57)<sec_prec)
  ...
  def test_range
    ...
    min1 = PDate.minute(2004,2,29,23,50,22)
    #12:02 am , March 1st, 2004
    min2 = PDate.minute(2004,3,1,0,2)
    #Inclusive Range w/minute precision
    r_min = min1..min2
    assert( r_min.include?(PDate.minute(2004,2,29,23,50,22)) )
    assert( r_min.include?(PDate.minute(2004,3,1,0,2)) )
    assert( r_min.include?(PDate.minute(2004,3,1,0,0)) )
    assert( ! r_min.include?(PDate.minute(2004,3,1,0,3)) )
  ...
  def test_create_with_class_methods
    ...
    day_prec = PDate.day_of_month(1968,12,12,11,15)
